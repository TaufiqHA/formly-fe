...
const orders = [
  { id: '#ORD-2023-0891', date: '12 Okt 2023, 14:30', name: 'Budi Santoso', form: 'Pemesanan Katering Event', status: 'Menunggu', statusColor: 'bg-surface-container-high text-on-surface-variant', dot: 'bg-outline' },
  { id: '#ORD-2023-0890', date: '12 Okt 2023, 11:15', name: 'Siti Aminah', form: 'Registrasi Workshop Mingguan', status: 'Diproses', statusColor: 'bg-secondary-container text-primary', dot: 'bg-primary' },
  { id: '#ORD-2023-0889', date: '11 Okt 2023, 09:45', name: 'Andi Wijaya', form: 'Pemesanan Merchandise Custom', status: 'Selesai', statusColor: 'bg-success-bg text-success-text', dot: 'bg-success-text' },
  { id: '#ORD-2023-0888', date: '10 Okt 2023, 16:20', name: 'Ratna Sari', form: 'Pemesanan Katering Event', status: 'Dibatalkan', statusColor: 'bg-error-container text-on-error-container', dot: 'bg-error' },
  { id: '#ORD-2023-0887', date: '10 Okt 2023, 13:00', name: 'PT Maju Mundur', form: 'Registrasi Workshop Mingguan', status: 'Selesai', statusColor: 'bg-success-bg text-success-text', dot: 'bg-success-text' },
];

export default function OrderQueue({ onSelectOrder }: { onSelectOrder: () => void }) {
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: 'Total Pesanan', value: '1,284', trend: '+12% vs last month', color: 'text-primary' },
          { label: 'Menunggu Konfirmasi', value: '42', trend: 'Requires attention', color: 'text-on-surface-variant' },
          { label: 'Sedang Diproses', value: '156', trend: 'In logistics network', color: 'text-on-surface-variant' },
          { label: 'Selesai', value: '1,086', trend: 'Successfully delivered', color: 'text-success-text' },
        ].map((stat, idx) => (
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-container-low border-b border-outline-variant">
                <th className="py-4 px-6 text-xs font-bold text-on-surface-variant uppercase tracking-widest">ID Pesanan</th>
                <th className="py-4 px-6 text-xs font-bold text-on-surface-variant uppercase tracking-widest text-center">Waktu</th>
                <th className="py-4 px-6 text-xs font-bold text-on-surface-variant uppercase tracking-widest text-center">Nama Pelanggan</th>
                <th className="py-4 px-6 text-xs font-bold text-on-surface-variant uppercase tracking-widest text-center">Form Sumber</th>
            ...
            <tbody className="text-sm text-on-surface divide-y divide-outline-variant">
              {orders.map((order, idx) => (
                <tr key={idx} className="hover:bg-surface-bright transition-colors">
                  <td className="py-4 px-6 whitespace-nowrap font-bold text-primary cursor-pointer hover:underline" onClick={onSelectOrder}>
                    {order.id}
                  </td>
                  <td className="py-4 px-6 whitespace-nowrap text-on-surface-variant text-center">{order.date}</td>
                  <td className="py-4 px-6 whitespace-nowrap text-center">{order.name}</td>
                  <td className="py-4 px-6 whitespace-nowrap text-center">{order.form}</td>
                  <td className="py-4 px-6 whitespace-nowrap text-center">
                    <span className={cn("inline-flex items-center px-3 py-1 rounded-full text-xs font-bold", order.statusColor)}>
                      <span className={cn("w-2 h-2 rounded-full mr-2", order.dot)}></span>
                      {order.status}
                    </span>
                  </td>
                  <td className="py-4 px-6 whitespace-nowrap text-right">
                    <div className="flex justify-end gap-2">
                      <button className="text-secondary hover:text-primary transition-colors p-1.5 rounded hover:bg-surface-container-low" onClick={onSelectOrder}>
                        <Eye size={18} />
                      </button>
